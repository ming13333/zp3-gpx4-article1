...
# -*- coding: utf-8 -*-
"""
Article 1 supplementary figure: ZP3 subtype direction chain (cross-cohort consistency)
===================================================
Core highlight: Finding #38, the "subtype direction chain" — ZP3 increases along the direction of worsening prognosis:
    G-CIMP (best prognosis/IDH-Mut) → Codel (1p19q co-deleted) → Non-codel → IDH-WT (worst prognosis)

This figure integrates 3 independent cohorts:
  · TCGA-GBM     Verhaak subtypes (G-CIMP group = IDH-Mut, best prognosis)
  · TCGA-LGG     1p19q Codel/Non-codel + IDH status
  · CGGA-693     IDH_mutation_status + 1p19q_codeletion_status (independent cohort)
                  plus IDH status (Wildtype/Mutant)

For cross-cohort comparability, the Y axis uses "fold change relative to the lowest group within each cohort" (absolute values in FPKM/TPM units are not directly comparable),
with the absolute ZP3 median of each cohort annotated secondarily.
...
# ---------- 1. Load the TCGA subtype data already produced in #38 ----------
...
# ---------- 3. Assemble direction chain (each level = (label, cohort, all ZP3 values at that level)) ----------
# TCGA-GBM subtypes (use median for G-CIMP, note unit is log2 TPM)
...
# Use the "molecular-pathology direction chain" (not transcriptomic subtypes, to avoid GBM/LGG transcriptomic direction contradictions):
# Sorting key: IDH-Mut (good prognosis) on the left, IDH-WT (poor prognosis) on the right
rows = [
    # IDH-Mut group (low ZP3)
    ...
    # IDH-WT group (high ZP3)
    chain("CGGA\nIDH-WT",  "CGGA", cg_idh_wt,   "#A32D2D"),
    chain("LGG\nIDH-WT",   "TCGA", lgg_wt,      "#D9534F"),
]

# ---------- 4. Fold change relative to the lowest group within each cohort ----------
# Within each cohort: select one group as baseline (TCGA uses G-CIMP; CGGA uses whichever is lower of Codel or IDH-Mut)
def cohort_baseline(cohort, rows):
    """Return the group with the lowest median ZP3 in that cohort."""
...
# Main panel: direction chain (original order) — X uses ordinal positions, ordered IDH-Mut→WT
x = np.arange(len(rec))
...
# Right panel: IDH-Mutant vs Wildtype fold-change comparison across cohorts
idh_pairs = [
...
# ---------- 6. Console summary ----------
print("=== Subtype direction chain values ===")
print(val_df.to_string(index=False))
print()
print("=== IDH-WT vs Mutant fold-change ===")
for cohort, mut, wt, lab in idh_pairs:
...
print("\nSaved: tcga_subtype_direction_chain.png + subtype_direction_chain_values.csv")
